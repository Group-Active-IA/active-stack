// Package headless implements the non-interactive install mode for active-stack.
...
// TestParseInstallFlags_Lang covers task 3.1: --lang es sets ParsedFlags.Lang,
// omitting --lang leaves Lang at the zero value (treated as i18n.Default by
// callers), and --lang xx is rejected with an error naming the value.
func TestParseInstallFlags_Lang(t *testing.T) {
	t.Run("--lang es sets Lang", func(t *testing.T) {
		result, err := headless.ParseInstallFlags([]string{"--mode", "lite", "--lang", "es"})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("--lang xx is rejected", func(t *testing.T) {
		_, err := headless.ParseInstallFlags([]string{"--mode", "lite", "--lang", "xx"})
		if err == nil {
			t.Fatal("ParseInstallFlags() expected error for --lang xx, got nil")
		}
