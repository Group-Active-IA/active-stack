// Command active-stack is the Active Stack installer — a methodology-first harness
// installer for AI coding agents.
//
// Usage:
//
//	active-stack install              Launch the interactive TUI install flow.
//	active-stack install --headless   Non-interactive install (also implied by --mode/--agent).
//	active-stack install --dry-run    Print the install plan; do not execute.
//	active-stack install --help       Show all available flags.
package main
...
func main() {
	...
	case "windows":
		...
		regWrapper := agentRegistryAdapter{r: reg}
		exitCode := runWindowsDispatch(os.Args[2:], cat, regWrapper, os.Stdout)
		if exitCode != 0 {
			os.Exit(exitCode)
		}
	default:
