...
| 11 | C-29 | C-31, C-32 | **HECHO** — subcomando `active-stack starter add` |
...
- **Estado**: HECHO (split A/B). Cierra el pipeline.
  ...
  - **Wave B** — modo headless del binario (resuelve D4: `active-stack install --headless --mode --agent --custom --dry-run --yes --home`, sin flags → TUI, cero regresión; commit `d34bb8b`) + suite E2E harness-first con matrix Docker Ubuntu+Arch (Go 1.26). **Tier 1 VERDE 15/15 en ambas distros.** Commit `6e3cd4c`.
...
- ~~**Nombre del repo remoto en GitHub**.~~ → **RESUELTO**: el repo viejo queda **archivado como legacy**; este conserva el nombre **`active-stack`** (binario y repo). Doc-only.
...
- **Dependencias**: C-02 (modelo + catálogo base). Habilita: C-27 (target proyecto en adapters), C-28 (escritura .mcp.json), C-29 (subcomando `active-stack starter add`), C-30 (publicar repos por starter), C-31 (slash-command), C-32 (verify + E2E starters).
...
- **Scope**: Infraestructura de "target de proyecto" para el epic starter packs. Habilita escribir harnesses bajo `<proyecto>/.claude/` (o `.opencode/`) en lugar del home. NO expone comando nuevo (eso es C-29).
  ...
  - Snapshot de proyecto bajo `<ProjectRoot>/.active-stack/backups/install`. Reusa `internal/backup` con `CreateWithDirHints` exactamente igual que el flujo de máquina. Skills dirs registradas como DirHints para rollback dir-aware (RemoveAll si fue creado por el install, NO-OP si preexistía).
...
- **Dependencias**: C-26 (modelo starters, base del epic). Habilita: C-28 (mcp scope proyecto), C-29 (subcomando `active-stack starter add`).
...
- **Dependencias**: C-26 (modelo + `model.MCP` placeholder), C-27 (`PathsFor` target-aware). Habilita: C-29 (subcomando `active-stack starter add`).
...
### C-29 — Subcomando `active-stack starter add`
...
- **Scope**: Primera superficie de usuario del epic. Expone `active-stack starter add <id> [--project <path>] [--dry-run] [--yes] [--agent <list>]`. NO construye maquinaria nueva — CABLEA el CLI al pipeline ya existente (C-26/C-27/C-28). Foco Claude + OpenCode.
  - Nuevo `case "starter"` en el dispatch manual (`switch os.Args[1]`) de `cmd/active-stack/main.go` — sin cobra, respeta el estilo de `case "install"`.
...
- **Archivos nuevos**: `internal/catalog/resolve_mcps_test.go`, `cmd/active-stack/headless/starter_flags.go` (+`_test`), `cmd/active-stack/headless/project_root_test.go`, `cmd/active-stack/headless/starter_executor_test.go`, `cmd/active-stack/starter_add.go` (+`_test`), `cmd/active-stack/starter_dispatch.go`, `cmd/active-stack/routing_test.go`.
- **Archivos modificados**: `internal/catalog/catalog.go` (`ResolveStarterMCPs` + `AllStarters`), `cmd/active-stack/headless/flags.go` (`ParsedFlags` += `Target`/`ProjectRoot`/`Starter`), `cmd/active-stack/headless/executor.go` (cablea los campos a `install.Options`), `cmd/active-stack/main.go` (`case "starter"`).
...
- **TBDs resueltos**:
  ...
  - D5: `install_modes: [full]` para todas las skills de starter (materializadas vía `active-stack starter add`, no por modo base).
...
- **Scope**: Shipea un slash-command fino que envuelve `active-stack starter add`. **Capacidad NUEVA**: el installer no shipeaba commands hasta ahora. Foco Claude + OpenCode.
  ...
  - **`$ARGUMENTS`** (TBD-1, verificado contra doc oficial de AMBOS agentes: code.claude.com/docs/en/agent-sdk/slash-commands + opencode.ai/docs/commands) → una sola implementación, sin branching en los bodies. El body es un wrapper fino que ejecuta `active-stack starter add $ARGUMENTS` (binario en PATH), sin reimplementar lógica.
...
- **Red test real (D2)**: `TestWireEmbeddedFS_SetsCommandsFS` en `cmd/active-stack/main_wiring_test.go` — falla en compilación en HEAD actual (`undefined: wireEmbeddedFS`), pasa después del fix. Usa `install.GetEmbeddedCommandsFS()` / `install.ResetEmbeddedCommandsFS()` (seams nuevos en `testseams.go`) y `assets.CommandsFS` real, no un `fstest.MapFS` handrolled.
...
- **Archivos nuevos**: `cmd/active-stack/main_wiring_test.go`, `internal/install/command_entrypoint_wiring_test.go`.
- **Archivos modificados**: `cmd/active-stack/main.go` (`wireEmbeddedFS()` + call en `main()`), `internal/install/testseams.go` (`GetEmbeddedCommandsFS` + `ResetEmbeddedCommandsFS`).
...
- **Regresión corregida**: `active-stack starter add <id>` fallaba con `error: build plan: install: resolve dependencies: planner: unknown harness "test-driven-development"`. 3 tests rojos en `cmd/active-stack`: `TestStarterDispatch_AddSubcommandRoutes`, `TestRunStarterAdd_KnownID_BuildsPlanWithProjectTarget`, `TestRunStarterAdd_DryRun_WritesNothing`.
...
- **Impacto en implementaciones de `install.Catalog`**: 4 fakes en tests necesitaban `AllHarnesses()` — añadido en `install_test.go` (`fakeCatalog`), `internal/tui/gate_test.go` (`fakeTUICatalog`), `internal/tui/review_test.go` (`fakeCatalog`), `cmd/active-stack/headless/executor_test.go` (`fakeExecCatalog`), `cmd/active-stack/unify_test.go` (`cmdFakeCatalog`). Todos implementan `return f.harnesses` (sin filtro).
...
- **Problema corregido**: `build.bat`/`build.sh` compilaban el binario a la raíz del repo, pero ningún paso copiaba el resultado al PATH del usuario. `active-stack` seguía resolviendo a un binario stale en `%LOCALAPPDATA%ctive-stackin` (o a nada). El usuario tenía que mover manualmente el `.exe` y re-ejecutar. Ahora `active-stack install` se auto-instala por defecto.
...
- **Contrato implementado**:
  ...
  - **(D5)** Flag `--no-self-install` en `cmd/active-stack/headless/flags.go` → `ParsedFlags.NoSelfInstall`. `RunHeadless` cablea a `opts.NoSelfInstall`. `main.go` resuelve `os.Executable()` una vez y lo pasa como `BinaryPath` en `ParsedFlags`.
  - **(TBD-2)** `build.bat` y `build.sh`: añadido hint "ejecuta `active-stack install` para terminar el setup" al final del output.
...
- **Archivos nuevos**: `e2e/starter/starter_e2e_helpers.go`, `e2e/starter/starter_hermetic_test.go`, `e2e/starter/starter_network_test.go` (`//go:build e2e_network`), `internal/catalog/parse_external.go` (`ParseForTest()` para catálogos de test sin tocar `harnesses.yaml`), `internal/pipeline/degraded_status_test.go`, `internal/install/degraded_result_test.go`, `cmd/active-stack/headless/degraded_executor_test.go`, `internal/tui/degraded_tui_test.go`.
- **Archivos modificados**: `internal/pipeline/result.go` (`StepStatusDegraded`), `internal/install/steps.go` (best-effort emite `Degraded`), `cmd/active-stack/headless/executor.go` (case `Degraded` `⚠` + bloque resumen + wiring `opts.OnProgress`), `internal/tui/model.go` (`statusIcon` + `viewComplete` con degradados), `internal/harness/skill/clone.go` (soporte `file://`), `internal/install/best_effort_test.go` (extendido).
